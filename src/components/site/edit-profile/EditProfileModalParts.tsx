...
interface EditProfilePartProps {
  form: EditProfileData
  t: TFunction
}

export function EditProfileMediaHeader({ form, onClose, t }: EditProfilePartProps & { onClose: () => void }) {
  return (
...
interface EditProfileFieldsProps extends EditProfilePartProps {
  onFieldChange: (key: keyof EditProfileData) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void
}

export function EditProfileFields({ form, onFieldChange, t }: EditProfileFieldsProps) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.username")}</label>
          <input value={form.username} onChange={onFieldChange("username")} placeholder={t("edit_profile.username")} className={inputCls} />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.birth_date")}</label>
          <input type="date" value={form.birthDate} onChange={onFieldChange("birthDate")} className={cn(inputCls, "[color-scheme:dark]")} />
        </div>
      </div>

      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.email")}</label>
        <input type="email" value={form.email} onChange={onFieldChange("email")} placeholder="your@email.com" className={inputCls} />
      </div>

      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.bio")}</label>
        <textarea rows={3} value={form.bio} onChange={onFieldChange("bio")} placeholder={t("edit_profile.bio_placeholder")} className={cn(inputCls, "resize-none leading-relaxed")} />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.avatar_url")}</label>
          <input value={form.avatarUrl} onChange={onFieldChange("avatarUrl")} placeholder="https://..." className={inputCls} />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t("edit_profile.banner_url")}</label>
          <input value={form.bannerUrl} onChange={onFieldChange("bannerUrl")} placeholder="https://..." className={inputCls} />
        </div>
      </div>
    </div>
  )
...
export function EditProfileActions({ hasChanged, onCancel, onSave, t }: EditProfileActionsProps) {
  return (

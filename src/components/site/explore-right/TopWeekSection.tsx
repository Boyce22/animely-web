    ...
    <div className="border-b border-white/[0.07] p-4">
      ...
      <div className="flex flex-col">
        {items.map(item => (
          ...
          >
            <span
              className={`text-[14px] font-black font-mono w-6 text-center flex-shrink-0 ${item.rank <= 2 ? "text-primary" : "text-white/30"}`}
            >
              {item.rank}
            </span>
            <span className="text-[14px] font-bold flex-1 min-w-0 truncate text-foreground/90">
              {item.title}
            </span>
            <span className="text-[13px] font-black font-mono text-muted-foreground/70 flex-shrink-0">
